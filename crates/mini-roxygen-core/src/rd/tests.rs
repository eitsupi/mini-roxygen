...
#[test]
fn explicit_doc_type_is_emitted_for_ordinary_topics() {
    let (model_output, sources) = model(
        r#"#' Function title
#' @docType class
f <- function() f
...
#[test]
fn explicit_doc_type_overrides_an_inferred_data_type() {
    let (model_output, sources) = model(
        r#"#' Dataset title
#' @docType class
"dataset"
"#,
    ...
    assert!(generated.content.contains(r"\docType{class}"));
    assert!(!generated.content.contains(r"\docType{data}"));
